...
export function AddContactModal({ onClose, onAdd }: Props) {
  ...
  return <div className={`modal-backdrop ${isClosing ? 'is-closing' : ''}`} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && requestClose()}><div className="modal-card" role="dialog" aria-modal="true" aria-labelledby="add-contact-title" ref={modalRef}>
    <div className="modal-header"><div className="modal-icon"><Icon name="user-plus" size={20} /></div><div><p>Nuevo registro</p><h2 id="add-contact-title">Agregar contacto</h2></div><button type="button" className="icon-button" onClick={requestClose} aria-label="Cerrar formulario"><Icon name="close" size={19} /></button></div>
    <Formik<ContactFormValues> initialValues={{ name: '', email: '', phone: '', department: '' }} validationSchema={contactSchema} validateOnMount validateOnChange validateOnBlur onSubmit={submit}>{({ isValid, dirty, isSubmitting, touched, errors, handleBlur, handleChange, values }) => <Form noValidate>
      ...
      <div className="modal-footer"><button className="secondary-button" type="button" onClick={requestClose}>Cancelar</button><button className="primary-button" type="submit" disabled={!dirty || !isValid || isSubmitting} aria-disabled={!dirty || !isValid || isSubmitting}><Icon name="plus" size={17} /> Agregar contacto</button></div>
    </Form>}</Formik>
  </div></div>;
}
