...
export function AddContactModal({ onClose, onAdd }: Props) {
  ...
  return (
    <div className={`modal-backdrop ${isClosing ? 'is-closing' : ''}`} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && requestClose()}>
      <div className="modal-card" role="dialog" aria-modal="true" aria-labelledby="add-contact-title" ref={modalRef}>
        <div className="modal-header">
          <div className="modal-icon"><Icon name="user-plus" size={20} /></div>
          <div><p>Nuevo registro</p><h2 id="add-contact-title">Agregar contacto</h2></div>
          <button type="button" className="icon-button" onClick={requestClose} aria-label="Cerrar formulario"><Icon name="close" size={19} /></button>
        </div>
