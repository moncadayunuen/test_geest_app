...
export function ContactManager() {
  ...
  return <main className="app-shell">
    ...
    <header className="topbar"><a className="brand" href="#main-content" aria-label="Nexo, ir al contenido"><span className="brand-mark"><Icon name="brand" size={20} /></span><span>Nexo</span></a><div className="topbar-actions"><span className="workspace-label">Espacio de trabajo</span><span className="avatar avatar-small">YM</span></div></header>
    <div className="layout"><aside className="sidebar" aria-label="Navegación principal"><nav><a className="nav-item active" href="#main-content" aria-current="page"><Icon name="users" size={19} /> Contactos</a></nav><div className="sidebar-note"><span className="status-dot" /><div><strong>Directorio activo</strong><span>Datos locales</span></div></div></aside>
      <section id="main-content" className="content">
        {showAnnouncement && <AnnouncementBanner count={contacts.length} onClose={() => setShowAnnouncement(false)} />}
        ...
        <section className="directory-card" aria-labelledby="directory-title">
          <div className="directory-toolbar"><div><h2 id="directory-title">Directorio</h2><p aria-live="polite">{loading ? 'Cargando contactos…' : `${filteredContacts.length} ${filteredContacts.length === 1 ? 'contacto encontrado' : 'contactos encontrados'}`}</p></div><div className="toolbar-actions"><div className="search-control"><span className="search-label">Buscar contactos</span><label className="search-field"><Icon name="search" size={20} /><span className="sr-only">Buscar por nombre</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Escribe un nombre…" /></label></div>{hasFilters && <button className="secondary-button clear-button" type="button" onClick={clearFilters}><Icon name="reset" size={14} /> Limpiar filtros</button>}</div></div>
    ...
    {pendingDelete && <ConfirmationModal title="Eliminar contacto" description={<>¿Seguro que quieres eliminar a <strong>{pendingDelete.name}</strong>? Esta acción no se puede deshacer.</>} icon={<Icon name="trash" size={22} />} confirmLabel="Sí, eliminar" onClose={() => setPendingDelete(null)} onConfirm={deleteContact} />}
    {message && <div className="toast" role="status"><Icon name="check" /><span>{message}</span><button onClick={clearMessage} aria-label="Cerrar notificación"><Icon name="close" size={15} /></button></div>}
  </main>;
}
